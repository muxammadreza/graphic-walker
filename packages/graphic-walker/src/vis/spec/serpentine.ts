...
export interface SerpentineSpecProps {
    ...
    dateField: IViewField | null;
    labelField: IViewField | null;
...
/**
 * Generates a Vega (not Vega-Lite) spec for a Serpentine Timeline visualization.
...
export function toSerpentineSpec(props: SerpentineSpecProps) {
    ...
    const dataset = dataSource
        .map((row, index) => {
            let domain: number;
            if (dateField) {
                const val = row[dateField.fid];
                if (typeof val === 'number') {
                    domain = val;
                } else if (typeof val === 'string' || val instanceof Date) {
                    const parsed = new Date(val).getTime();
                    domain = isNaN(parsed) ? index : parsed;
                } else {
                    domain = index;
                }
            } else {
            ...
            const label = labelField ? String(row[labelField.fid] ?? '') : String(domain);

            return {
                domain,
                label,
                ...(colorField ? { color: row[colorField.fid] } : {}),
            };
        })
        .filter((d) => d.domain !== null && d.domain !== undefined)
                ...
                ? [
                      {
                          name: 'color',
                          type: 'ordinal',
                          domain: { data: 'milestones', field: 'color' },
                          range: 'category',
                      },
                  ]
                : []),
            ...
            },
            {
                ...
                encode: {
                    update: {
                        ...
                        y: { field: 'y' },
                        dy: { field: 'dy' },
                        text: { signal: "datum['label'] && datum['label'] !== '' ? datum['label'] : timeFormat(datum['domain'], '%Y-%m-%d %H:%M')" },
                        fontSize: { signal: '10' },
                        fill: { value: '#fff' },
                        stroke: { value: '#fff' },
                        strokeWidth: { value: 7 },
            ...
            },
            {
                ...
                encode: {
                    update: {
                        ...
                        text: { signal: "datum['label'] && datum['label'] !== '' ? datum['label'] : timeFormat(datum['domain'], '%Y-%m-%d %H:%M')" },
                        fontSize: { signal: '10' },
                        fill: colorField ? { scale: 'color', field: 'color' } : { value: 'firebrick' },
                        angle: { field: 'labelAngle' },
                        align: { value: 'center' },
                        baseline: {
                            signal: "datum['type'] === 'straight' ? 'top' : 'bottom'",
        ...
        data: [
            ...
            },
            {
                ...
                transform: [
                    {
                        type: 'formula',
                        expr: 'sequence(sDomain[0],sDomain[1], max((sDomain[1] - sDomain[0]) / 1000, 0.1))',
                        as: 'domain',
                    },
            ...
            },
            {
                ...
                values: [{ category: 'start' }, { category: 'serpentine' }, { category: 'milestone' }, { category: 'tick' }, { category: 'end' }],
                transform: [
